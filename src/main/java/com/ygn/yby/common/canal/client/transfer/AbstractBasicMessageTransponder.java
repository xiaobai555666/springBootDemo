...
import com.ygn.yby.common.canal.client.exception.CanalClientException;
...
public abstract class AbstractBasicMessageTransponder extends AbstractMessageTransponder {
    ...
    public AbstractBasicMessageTransponder(CanalConnector connector, Map.Entry<String, CanalConfig.Instance> config, List<CanalEventListeners> listeners, List<ListenerPoint> annoListeners) {
    ...
    protected void distributeByImpl(CanalEntry.EventType eventType, CanalEntry.RowData rowData) {
        if (listeners != null) {
            for (CanalEventListeners listener : listeners) {
